...
getIrisLabels :: [Iris] -> LA.Vector LA.R
getIrisLabels irises = LA.fromList $ irisDoubles
  where irisDoubles = map (fromIntegral . parseIrisClass . irisClass) irises

getIrisFeatures :: [Iris] -> LA.Matrix LA.R
getIrisFeatures irises = LA.tr $ LA.fromLists [sepalLengths, sepalWidths, petalLengths, petalWidths]
...
getIris :: [Iris] -> (LA.Matrix LA.R, LA.Vector LA.R)
...
type FFNet = Network '[ FullyConnected 4 40,
                        Tanh,
                        FullyConnected 40 10,
                        Relu,
                        FullyConnected 10 1,
                        Logit ]
                     '[ 'D1 4, 'D1 40, 'D1 40, 'D1 10, 'D1 10, 'D1 1, 'D1 1]
...
--convertHMatrix :: LA.Matrix LA.R -> S n
--convertHMatrix mat = S2D mat

netTrainNew :: FFNet -> LearningParameters -> LA.Matrix LA.R -> LA.Vector LA.R -> Int -> IO FFNet
netTrainNew net0 params features labels n = do
    inps <- replicateM n $ do
      s  <- getRandom
      return $ S1D $ SA.randomVector s SA.Uniform * 2 - 1
    let outs = flip map inps $ \ _ ->
                 S1D $ (SA.randomVector 0 SA.Uniform :: SA.R 1)
    let trained = foldl' trainEach net0 (zip inps outs)
    return trained
  where
    trainEach !network (i,o) = train params network i o
